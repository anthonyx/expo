#import <ABI34_0_0EXImagePicker/ABI34_0_0EXImagePicker.h>
...
const CGFloat ABI34_0_0EXDefaultImageQuality = 0.2;

@interface ABI34_0_0EXImagePicker ()
...
@property (nonatomic, weak) id<ABI34_0_0UMPermissionsInterface> permissionsModule;
@property (nonatomic, assign) BOOL shouldRestoreStatusBarVisibility;

@end

@implementation ABI34_0_0EXImagePicker

ABI34_0_0UM_EXPORT_MODULE(ExponentImagePicker);
...
- (void)setModuleRegistry:(ABI34_0_0UMModuleRegistry *)moduleRegistry
{
  ...
  _permissionsModule = [self.moduleRegistry getModuleImplementingProtocol:@protocol(ABI34_0_0UMPermissionsInterface)];
}

ABI34_0_0UM_EXPORT_METHOD_AS(launchCameraAsync, launchCameraAsync:(NSDictionary *)options
                  ...
                  rejecter:(ABI34_0_0UMPromiseRejectBlock)reject)
{
  ...
  self.reject = reject;
  [self launchImagePicker:ABI34_0_0EXImagePickerTargetCamera options:options];
}

ABI34_0_0UM_EXPORT_METHOD_AS(launchImageLibraryAsync, launchImageLibraryAsync:(NSDictionary *)options
                  resolver:(ABI34_0_0UMPromiseResolveBlock)resolve
                  rejecter:(ABI34_0_0UMPromiseRejectBlock)reject)
{
  ...
  self.resolve = resolve;
  self.reject = reject;
  [self launchImagePicker:ABI34_0_0EXImagePickerTargetLibrarySingleImage options:options];
}

- (void)launchImagePicker:(ABI34_0_0EXImagePickerTarget)target options:(NSDictionary *)options
{
...
- (void)launchImagePicker:(ABI34_0_0EXImagePickerTarget)target
{
  self.picker = [[UIImagePickerController alloc] init];

  if (target == ABI34_0_0EXImagePickerTargetCamera) {
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    id<ABI34_0_0UMUtilitiesInterface> utils = [self.moduleRegistry getModuleImplementingProtocol:@protocol(ABI34_0_0UMUtilitiesInterface)];
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
  dispatch_block_t dismissCompletionBlock = ^{
    ...
    id<ABI34_0_0UMFileSystemInterface> fileSystem = [self.moduleRegistry getModuleImplementingProtocol:@protocol(ABI34_0_0UMFileSystemInterface)];
    if (!fileSystem) {
          ...
          completionHandler:(void (^)(void))completionHandler
{
  ...
  NSData *data = UIImageJPEGRepresentation(image, quality == nil ? ABI34_0_0EXDefaultImageQuality : [quality floatValue]);
  ...
  if ([[self.options objectForKey:@"exif"] boolValue]) {
    ...
    if (metadata) {
      ...
      if (assets.count > 0) {
        ...
        ABI34_0_0UMLogInfo(@"Could not fetch metadata for image %@", [imageURL absoluteString]);
        completionHandler();
      }
    }
  } else {
...
- (void)handleVideoWithInfo:(NSDictionary * _Nonnull)info saveAt:(NSString *)directory updateResponse:(NSMutableDictionary *)response
{
  ...
  if (info[UIImagePickerControllerReferenceURL]) { // video from gallery
    ...
    if (assets.count > 0) {
      ...
      ABI34_0_0UMLogInfo(@"Could not fetch metadata for video %@", [videoURL absoluteString]);
    }
  }
